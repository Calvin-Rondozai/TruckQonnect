...
import { AUTH, maskEmailAddress } from '@/components/truckq/auth/auth-theme';
import { TQFonts } from '@/constants/truckq-design';
import { ApiError, resendOtp, verifyOtp, type VerifyOtpResponse } from '@/lib/auth-api';
...
type Props<T = VerifyOtpResponse> = {
  email: string;
  emailMasked?: string;
  ...
  verifyFn?: (email: string, code: string) => Promise<T>;
  resendFn?: (email: string) => Promise<unknown>;
  devOtp?: string;
};

export function OtpScreen<T = VerifyOtpResponse>({
  email,
  emailMasked,
  onVerified,
  onChangeEmail,
  ...
  const [devCode, setDevCode] = useState(devOtp ?? '');
  ...
  const displayEmail = emailMasked || maskEmailAddress(email);
  ...
  const verify = useCallback(
    async (code: string) => {
      ...
      try {
        ...
        setHasError(true);
        setErrorMsg(
          e instanceof ApiError ? e.message : 'Invalid verification code. Please try again.'
        );
        shake();
    ...
    [email, onVerified, shake, verifyFn]
  );
  ...
  const handleChange = (text: string) => {
    ...
    if (digits.length === OTP_LENGTH) {
      setTimeout(() => void verify(digits.join('')), 200);
    }
  };
  ...
  const handleResend = async () => {
    ...
    try {
      if (resendFn) {
        const result = (await resendFn(email)) as { dev_otp?: string };
        if (result?.dev_otp) setDevCode(result.dev_otp);
      } else {
      ...
      setHasError(true);
      setErrorMsg(e instanceof ApiError ? e.message : 'Could not resend code. Try again later.');
    }
  };
      ...
      >
        ...
        <View style={styles.inner}>
          ...
          >
            ...
            <TouchableOpacity style={styles.changeNumBtn} onPress={onChangeEmail}>
              <Text style={styles.changeNumText}>Change email</Text>
            </TouchableOpacity>
          </Animated.View>
        </View>
      </KeyboardAvoidingView>
