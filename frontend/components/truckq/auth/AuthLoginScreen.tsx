...
import { ApiError, loginAccount, type LoginResponse } from '@/lib/auth-api';
import type { UserRole } from '@/lib/owner-types';

const LOGO = require('@/assets/images/logo.png.jpeg');

export type AuthOtpParams = {
  email: string;
  emailMasked: string;
  role: UserRole;
  devOtp?: string;
...
type Props = {
  role: UserRole;
  onLoginSuccess: (result: LoginResponse) => void | Promise<void>;
};

export function AuthLoginScreen({ role, onLoginSuccess }: Props) {
  ...
  const submit = async () => {
    ...
    setError('');
    try {
      const res = await loginAccount(email.trim().toLowerCase(), password, role);
